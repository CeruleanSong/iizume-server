module Helper
	module Cache
		def Cache.find_chapter_by_number(manga_id, chapter_number)
			return $DB[:chapter]
		...
		def Cache.find_manga_by_id(source_id, manga_id)
			return $DB[:manga]
			.where(Sequel.or(manga_id: manga_id))
			.natural_join(:source_manga).first
		end

		def Cache.find_manga_by_origin(source_id, manga_origin)
			return $DB[:manga]
			.where(Sequel.or(origin: manga_origin))
			.natural_join(:source_manga).first
		end

		def Cache.find_manga_by_origin_or_title(source_id, manga_origin, manga_title)
		...
		def Cache.find_manga_by_title(source_id, manga_title)
			return $DB[:manga]
		...
		def Cache.store_manga(source_id, manga_info)
			$DB[:manga]
		...
		def Cache.store_manga_chapter(manga_id, chapter_info)
			$DB[:chapter]
			.on_duplicate_key_update()
			.insert(chapter_info)
			$DB[:manga_chapter]
		...
		def Cache.store_tags(manga_id, tag_list)
			for tag in tag_list
